...
# build is already on the workers.
#
# Only tests that PASSED the main pass are repeated: a new test that failed already gets
# --fail_repetitions re-runs, and repeating only the passing ones is what lets a consumer read
# retry_kind=new_test_repetition on an item as "the first attempt passed" (csi_report.py).
#
...
def run_new_test_repetitions(args: argparse.Namespace,
    ...
    num_new_failed = 0
    for result in results:
        if result.test_descriptor.descriptor_str_without_attempt_index in known:
            continue
    ...
    logging.info("New tests on this lane: %d passed, %d failed (the failed ones are re-run by "
                 "--fail_repetitions)", len(new_passed), num_new_failed)
    if not new_passed:
        return
    ...
    # repetitions of a new test, not first attempts and not fail repetitions.
...
def main() -> None:
    ...
    try:
        run_new_test_repetitions(args, conf, results, propagated_env_vars)
    except Exception:
